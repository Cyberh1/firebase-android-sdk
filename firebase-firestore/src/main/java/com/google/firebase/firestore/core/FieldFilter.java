...
import static com.google.firebase.firestore.model.value.FieldValue.TYPE_ORDER_REFERENCE;
...
public class FieldFilter extends Filter {
  ...
  public static FieldFilter create(FieldPath path, Operator operator, FieldValue value) {
    if (path.isKeyField()) {
      // TODO(mrschmidt): Move all asserts to FieldFilter classes
      if (operator == Operator.IN) {
        hardAssert(
            value.typeOrder() == TYPE_ORDER_ARRAY,
            "Comparing on key with IN, but the value was not an ArrayValue");
        return new KeyFieldInFilter(path, value);
      } else {
        hardAssert(
            value.typeOrder() == TYPE_ORDER_REFERENCE,
    ...
    } else if (value.typeOrder() == TYPE_ORDER_NULL) {
    ...
    } else if (Double.isNaN(value.getProto().getDoubleValue())) {
      if (operator != Filter.Operator.EQUAL) {
        throw new IllegalArgumentException(
